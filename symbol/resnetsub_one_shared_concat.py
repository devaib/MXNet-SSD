"""
Resnet with 2 sub network and shared first few layers
"""
import mxnet as mx
...
def resnetsub(units, num_stages, filter_list, num_classes, image_shape, bottle_neck=True, bn_mom=0.9, workspace=256, memonger=False):
    ...
    if height <= 32:            # such as cifar10
        ...
        body_sub1 = mx.symbol.Pooling(data=body, kernel=(3, 3), stride=(2,2), pad=(1,1), pool_type='max')

    num_shared_stages = 1   # amount of shared stages, 1 stage -> 3 _plus layer
    for i in range(num_stages):
        body_sub1 = residual_unit(body_sub1, filter_list[i+1], (1 if i==0 else 2, 1 if i==0 else 2), False,
                             name='stage%d_unit%d' % (i + 1, 1), bottle_neck=bottle_neck, workspace=workspace,
                             memonger=memonger)
        for j in range(units[i]-1):
            body_sub1 = residual_unit(body_sub1, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i + 1, j + 2),
                                 bottle_neck=bottle_neck, workspace=workspace, memonger=memonger)

        # share layers end
        if i == (num_shared_stages - 1):
            separete_from = body_sub1

    bn1 = mx.sym.BatchNorm(data=body_sub1, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn1')
    ...
    body_sub2 = separete_from
    for i in range(num_stages):
        if i < num_shared_stages:
            continue
